...
import { DollarSign, TrendingUp, PieChart as PieIcon, Globe, FileDown, ArrowUpRight, ArrowDownRight, Award } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card className="p-6 glass-card bg-gradient-to-br from-status-available/10 to-transparent border-status-available/20">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-muted-foreground uppercase font-black tracking-widest">Total Net Profit</p>
                            <h2 className="text-3xl font-black mt-2 text-status-available">${netProfit.toLocaleString()}</h2>
                        </div>
                        <div className="p-3 bg-status-available/20 rounded-2xl text-status-available animate-bounce-subtle">
                            <DollarSign className="h-6 w-6" />
                        </div>
                    </div>
                    <div className="mt-4 flex items-center gap-2 text-xs font-bold text-status-available">
                        <ArrowUpRight className="h-4 w-4" /> PERFORMANCE INDEX: STRONG
                    </div>
                </Card>
                ...
                <Card className="p-6 glass-card bg-gradient-to-br from-primary/10 to-transparent border-primary/20">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <h2 className="text-3xl font-black mt-2 text-primary">{margin}%</h2>
                        </div>
                    ...
                    <div className="mt-4 flex items-center gap-2 text-xs font-bold text-primary">
                        <Badge variant="outline" className="border-primary/30 text-primary uppercase text-[9px]">Target: 65%</Badge>
                    </div>
                </Card>
                ...
                <Card className="p-6 glass-card bg-gradient-to-br from-status-on-trip/10 to-transparent border-status-on-trip/20">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <h2 className="text-3xl font-black mt-2 text-status-on-trip">${totalRevenue.toLocaleString()}</h2>
                        </div>
                        <div className="p-3 bg-status-on-trip/20 rounded-2xl text-status-on-trip">
                            <Globe className="h-6 w-6" />
                        </div>
                    </div>
                    <div className="mt-4 flex items-center gap-2 text-xs font-bold text-status-on-trip uppercase">
                ...
                <Card className="p-6 glass-card relative overflow-hidden">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={regionalData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <XAxis dataKey="name" stroke="hsl(215 20% 55%)" fontSize={12} axisLine={false} tickLine={false} />
                            <YAxis stroke="hsl(215 20% 55%)" fontSize={12} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v / 1000}k`} />
                            <Tooltip
                ...
                <Card className="p-4 border-primary/30 bg-primary/5 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">The <strong>{regionalData.sort((a, b) => b.profit - a.profit)[0].name} Region</strong> is currently delivering the highest ROI at {((regionalData.sort((a, b) => b.profit - a.profit)[0].profit / regionalData.sort((a, b) => b.profit - a.profit)[0].revenue) * 100).toFixed(1)}%.</p>
                        </div>
                    </div>
